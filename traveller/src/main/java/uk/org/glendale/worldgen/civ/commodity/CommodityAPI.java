...
public class CommodityAPI {
	...
	/**
	 * Produce goods from a planet's resource. Returns a list of goods which can
	 * be added to the planet's goods list. There are no side effects to this
	 * method - it only works out what needs to be done, but doesn't actually
	 * persist any changes itself.
	 * 
	 * @param planet
	 *            Planet to produce goods for.
	 * @param resource
	...
	public List<TradeGood> produceFromResource(Planet planet, Resource resource) {
		...
		System.out.println("produceFromResource: [" + base.getName() + "] ["
				+ density + "]");
		...
		if (base.hasCode(CommodityCode.PR)) {
			...
			for (Commodity child : children) {
				...
				if (producedChild > 0) {
					...
					goods.put(
							child,
							new TradeGood(child, producedChild, child.getCost()));
				}
			}
		}
		...
		for (Resource r : list) {
			...
			if (r.getCommodity().getName().equals("Mammals")) {
				...
				for (TradeGood tg : goods) {
					System.out.println(" --> " + tg.getCommodity().getName()
							+ ": " + tg.getQuantity());
				}
			}
		}
