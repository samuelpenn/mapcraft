...
import uk.org.glendale.rpg.traveller.worlds.WorldBuilder.Terrain;
 ...
 * @author Samuel Penn.
 */
public class MacroJovian extends WorldBuilder {
	public MacroJovian() {
		super();
	}
	
	public MacroJovian(int width, int height) {
		super(width, height);
	}
	
	public void generate() {
		Terrain[]	bands = new Terrain[] { Terrain.RedGas, Terrain.YellowGas, Terrain.OrangeGas,
											Terrain.WhiteGas, Terrain.CreamGas, Terrain.OrangeGas,
											Terrain.RedGas, Terrain.OrangeGas, Terrain.OrangeGas,
											Terrain.RedGas, Terrain.CreamGas, Terrain.OrangeGas,
											Terrain.YellowGas, Terrain.RedGas, Terrain.OrangeGas,
											Terrain.YellowGas, Terrain.CreamGas, Terrain.CreamGas };
		double[]	heights = new double[bands.length];
		
		// Create random 'heights' for each band, which controls the darkness.
		for (int i=0; i < heights.length; i++) {
			heights[i] = Die.d10(2) / 15.0;
		}
