...
public abstract class FacilityBuilder {
	...
	/**
	 * Gets a value from the given option by key. First looks for the key with
	 * the population modifier, and if that fails then looks for the key by
	 * itself.
	 * 
	 * @param key
	 *            Key prefix to look for.
	...
	protected long getPopulation() {
		...
		for (int i = 0; i < attempts; i++) {
			...
			case 2:
				...
				break;
			default:
				number = (1000 + Die.rollZero(9000) / divisor)
						* (long) Math.pow(10, code - 3);
				break;
			}
			if (number > maximum) {
				maximum = number;
	 ...
	 * 
	 * @param residential
	 *            Type of residential facility to add.
	 * @param residentialSize
	 *            Base size of the facility.
	 */
	 ...
	 * facilities should add up to about 100 (modified by technology level). The
	 * base size of the provided facilities will be re-scaled to fit. A facility
	 * of size 100, plus a facility of size 50 would be rescaled to be about
	 * size 66 and 33 for example.
	 * 
	 * @param residential
	 * @param residentialSize
	 * @param installations
	...
	public void generate() {
		...
		GovernmentType government = getGovernment();
		int techLevel = getTechLevel();
		int lawLevel = getLawLevel();

		// Both TL and Law can be modified by the government type.
		techLevel += government.getTechModifier();
		lawLevel += government.getLawModifier();

		planet.setGovernment(government);
		planet.setTechLevel(techLevel);
		planet.setLawLevel(lawLevel);
	...
	public FacilityBuilder(FacilityFactory factory, Planet planet,
